...
import { levaStore, folder } from 'leva'
import { useDeepMemo } from 'leva/src/hooks'
import { useRenderRoot } from 'leva/src/components/Leva'
import { getValuesForPaths } from 'leva/src/utils'
import type { FolderSettings, Schema, SchemaToValues, Data, StoreType, OnChangeHandler } from 'leva/src/types'
...
export function useValuesForPath(store: StoreType, paths: string[], initialData: Data) {
  const valuesForPath = store.useStore((s) => {
    // i have to be careful which ones i overwrite specifically
